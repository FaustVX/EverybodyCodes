...
public sealed class Solution : ISolution
{
    ...
    {
        input = string.Create(input.Length * (Globals.IsTest ? Globals.Args!["repeat"].Get<int>() : 1000), input, (s, i) =>
        {
            while (!s.IsEmpty)
            {
                i.CopyTo(s);
        ...
        var distance = Globals.IsTest ? Globals.Args!["distance"].Get<int>() : 1000;
        var total = 0;
        for (var i = 0; i < input.Length; i++)
        {
            if (!char.IsLower(input[i]))
